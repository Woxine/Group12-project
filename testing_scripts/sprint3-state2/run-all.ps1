...
$logFile = Join-Path $reportsDir "sprint3-state2-all-$timestamp.log"
...
Write-Host "[Sprint3-State2] Running all unit tests..."
Write-Host "[Sprint3-State2] Log file: $logFile"
Write-Host "[Sprint3-State2] Maven command: $mavenCmd"
...
if ($exitCode -ne 0) {
    Write-Host "[Sprint3-State2] FAILED (exit code: $exitCode)" -ForegroundColor Red
    exit $exitCode
}

Write-Host "[Sprint3-State2] PASSED" -ForegroundColor Green
Write-Host "[Sprint3-State2] Full output saved to: $logFile"
