...
param(
    ...
    [ValidateSet("id14_priority_feedback", "id15_high_priority_issues", "id20_popular_rental_dates")]
    [string]$Feature,
    [string]$ProjectRoot = ""
)
...
$logFile = Join-Path $reportsDir "sprint3-state2-$Feature-$timestamp.log"
...
Write-Host "[Sprint3-State2][$Feature] Running targeted unit tests..."
Write-Host "[Sprint3-State2][$Feature] Test selector: $testSelector"
Write-Host "[Sprint3-State2][$Feature] Log file: $logFile"
Write-Host "[Sprint3-State2][$Feature] Maven command: $mavenCmd"
...
if ($exitCode -ne 0) {
    Write-Host "[Sprint3-State2][$Feature] FAILED (exit code: $exitCode)" -ForegroundColor Red
    exit $exitCode
}

Write-Host "[Sprint3-State2][$Feature] PASSED" -ForegroundColor Green
Write-Host "[Sprint3-State2][$Feature] Full output saved to: $logFile"
