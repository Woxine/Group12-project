...
$logFile = Join-Path $reportsDir "sprint3-state1-all-$timestamp.log"
...
Write-Host "[Sprint3-State1] Running all unit tests..."
Write-Host "[Sprint3-State1] Log file: $logFile"
Write-Host "[Sprint3-State1] Maven command: $mavenCmd"
...
if ($exitCode -ne 0) {
    Write-Host "[Sprint3-State1] FAILED (exit code: $exitCode)" -ForegroundColor Red
    exit $exitCode
}

Write-Host "[Sprint3-State1] PASSED" -ForegroundColor Green
Write-Host "[Sprint3-State1] Full output saved to: $logFile"
