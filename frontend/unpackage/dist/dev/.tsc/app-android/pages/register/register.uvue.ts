...
// --- Data ---
type FormData = { __$originalPosition?: UTSSourceMapPosition<"FormData", "pages/register/register.uvue", 78, 6>;
	name: string
	email: string
	password: string
}
...
// Server configuration - fixed value
const serverUrl = 'http://192.168.16.2:8080/api/v1/users';
...
const handleRegister = async (): Promise<void> => {
	...
	const payload = {__$originalPosition: new UTSSourceMapPosition("payload", "pages/register/register.uvue", 161, 8),
		name: formData.value.name.trim(),
		email: formData.value.email.trim(),
		password: formData.value.password
	};

	console.log('Attempting to register with payload:', payload, " at pages/register/register.uvue:167");
	...
	try {
		...
		uni.request({
			...
			success: (response) => {
				console.log('Registration API success response:', response, " at pages/register/register.uvue:181");

				if (response.statusCode >= 200 && response.statusCode < 300) {
					// Handle the response data without type assertion
					const result = response.data as UTSJSONObject; // This is now of type UTSJSONObject
					console.log('Registration successful:', result, " at pages/register/register.uvue:186");
					// Access the response properties dynamically if they exist
					const nameValue = result['name']
					const welcomeName = (nameValue != null) ? nameValue.toString() : formData.value.name; // Fallback to entered name
					...
					console.error('Registration failed (HTTP Status):', errorMessage, " at pages/register/register.uvue:244");
					showToast(errorMessage);
				}
			},
			fail: (error) => {
				console.error('Network error during registration:', error, " at pages/register/register.uvue:249");
		...
		console.error('Unexpected error during registration attempt:', error, " at pages/register/register.uvue:265");
    ...
    _cE("view", _uM({ class: "back" }), [
      ...
      _cE("view", _uM({ class: "login_container" }), [
        ...
        _cE("view", _uM({ class: "validation-info" }), [
          _cE("text", _uM({
            class: _nC(_uM({'valid': isNameValid.value, 'invalid': formData.value.name != '' && !isNameValid.value}))
          }), _tD(formData.value.name != '' ? (isNameValid.value ? '✓ Name is valid' : '✗ Name cannot be empty') : 'Name required'), 3 /* TEXT, CLASS */),
          _cE("text", _uM({
            class: _nC(_uM({'valid': isEmailValid.value, 'invalid': formData.value.email != '' && !isEmailValid.value}))
          }), _tD(formData.value.email != '' ? (isEmailValid.value ? '✓ Valid email format' : '✗ Invalid email format') : 'Email required'), 3 /* TEXT, CLASS */),
          _cE("text", _uM({
            class: _nC(_uM({'valid': isPasswordValid.value, 'invalid': formData.value.password != '' && !isPasswordValid.value}))
          ...
          _cE("text", _uM({
            class: _nC(_uM({'valid': passwordsMatch.value, 'invalid': confirmPassword.value != '' && !passwordsMatch.value}))
