...
const onSubmit = () => {
	...
	const body: UTSJSONObject = {__$originalPosition: new UTSSourceMapPosition("body", "pages/admin/modify.uvue", 105, 8),}
	let hasField = false
	...
	uni.request({
		url: buildApiUrl('/api/v1/scooters/' + idStr),
		method: 'PUT',
		data: body,
		header: {
...
return (): any | null => {
  ...
  return _cE("view", _uM({ class: "container" }), [
    ...
    _cE("view", _uM({ class: "form-section" }), [
      ...
      _cE("view", _uM({ class: "form-row" }), [
        _cE("text", _uM({ class: "label" }), "Location Latitude"),
        _cE("input", _uM({
          class: "input",
      ...
      _cE("view", _uM({ class: "form-row" }), [
        _cE("text", _uM({ class: "label" }), "Location Longitude"),
        _cE("input", _uM({
          class: "input",
      ...
      _cE("button", _uM({
        ...
        onClick: onSubmit
      }), _tD(isSubmitting.value ? 'Updating...' : 'Update Scooter'), 9 /* TEXT, PROPS */, ["disabled"])
    ])
  ])
}
}
