    ...
    companion object {
        ...
        var setup: (__props: GenPagesLoginLogin) -> Any? = fun(__props): Any? {
            ...
            val onSubmit = fun(){
                ...
                uni_request<Any>(RequestOptions(url = BASE_URL + "/api/v1/auth/login", method = "POST", data = object : UTSJSONObject() {
                    ...
                    if (res.statusCode === 200) {
                        ...
                        if (loginData != null) {
                            ...
                            if (token != null) {
                                ...
                                val userInfoData: UTSJSONObject = object : UTSJSONObject(UTSSourceMapPosition("userInfoData", "pages/login/login.uvue", 165, 13)) {
                                    var name = userName
                                    var email = userEmail
                                }
                                try {
                                ...
                                val rawRole = if (loginData["role"] != null) {
                                    loginData["role"]
                                } else {
                                    loginData["userRole"]
                                }
                                val roleStr = if (rawRole != null) {
                                    ("" + rawRole).trim()
                                } else {
                                    ""
                                }
                                val role = if (roleStr.length > 0) {
                                    roleStr.toUpperCase()
                                } else {
                                    "CUSTOMER"
                                }
                                try {
                                    uni_setStorageSync("role", role)
                                }
                                 catch (e: Throwable) {
                                    console.error("Failed to save role:", e, " at pages/login/login.uvue:185")
                                }
                                val roleForCheck = ("" + role).trim().toUpperCase()
                                val isAdmin = roleForCheck.indexOf("ADMIN") === 0 && roleForCheck.length === 5
                                val targetUrl = if (isAdmin) {
                                    "/pages/admin/home"
                                } else {
                                    "/pages/index/index"
                                }
                                console.log("Login role:", roleForCheck, "isAdmin:", isAdmin, "targetUrl:", targetUrl, " at pages/login/login.uvue:191")
                                uni_showToast(ShowToastOptions(title = if (isAdmin) {
                                    "Admin login"
                                } else {
                                    "Login successful!"
                                }
                                , icon = "success"))
                                setTimeout(fun(){
                                    uni_reLaunch(ReLaunchOptions(url = targetUrl))
                                }
                                , 800)
                                return
                            }
