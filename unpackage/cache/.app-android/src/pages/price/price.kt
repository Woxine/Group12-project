    ...
    companion object {
        ...
        var setup: (__props: GenPagesPricePrice) -> Any? = fun(__props): Any? {
            ...
            val confirm = fun(){
                ...
                }, success = fun(checkRes){
                    var hasActiveBooking = false
                    if (checkRes.statusCode >= 200 && checkRes.statusCode < 300 && checkRes.data != null && UTSAndroid.`typeof`(checkRes.data) === "object") {
                        val obj = checkRes.data as UTSJSONObject
                        val list = obj["data"]
                        if (list != null && UTSArray.isArray(list)) {
                            val arr = list as UTSArray<UTSJSONObject>
                            run {
                                var i: Number = 0
                                while(i < arr.length){
                                    val rawStatus = arr[i]["status"]
                                    val s = if (rawStatus != null) {
                ...
                , fail = fun(_){
                    uni_showToast(ShowToastOptions(title = "Failed to check bookings", icon = "none"))
                }
                ))
            }
            val cancel = fun(){
