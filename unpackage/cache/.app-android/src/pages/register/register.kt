        ...
        var setup: (__props: GenPagesRegisterRegister) -> Any? = fun(__props): Any? {
            ...
            val handleRegister = fun(): UTSPromise<Unit> {
                return wrapUTSPromise(suspend w1@{
                        ...
                        val payload: UTSJSONObject = object : UTSJSONObject(UTSSourceMapPosition("payload", "pages/register/register.uvue", 160, 8)) {
                            var name = formData.value.name.trim()
                            var email = formData.value.email.trim()
                            var password = formData.value.password
                        }
                        console.log("Attempting to register with payload:", payload, " at pages/register/register.uvue:166")
                        try {
                            isSubmitting.value = true
                            uni_request<Any>(RequestOptions(url = BASE_URL + "/api/v1/users", method = "POST", data = payload, header = object : UTSJSONObject() {
                                var `Content-Type` = "application/json"
                            }, timeout = 15000, success = fun(response){
                                console.log("Registration API success response:", response, " at pages/register/register.uvue:180")
                                if (response.statusCode >= 200 && response.statusCode < 300) {
                                    val resultObj = response.data as UTSJSONObject
                                    console.log("Registration successful:", resultObj, " at pages/register/register.uvue:185")
                                    ...
                                    if (regData != null && regData["id"] != null) {
                                        ...
                                        try {
                                            ...
                                            console.log("Saved userId from register:", registeredId, " at pages/register/register.uvue:201")
                                        }
                                         catch (e: Throwable) {
                                            console.error("Failed to save userId from register:", e, " at pages/register/register.uvue:203")
                                        }
                                    }
                                    ...
                                    console.error("Registration failed (HTTP Status):", errorMessage, " at pages/register/register.uvue:267")
                                    showToast(errorMessage)
                                }
                            }
                            , fail = fun(error){
                                console.error("Network error during registration:", error, " at pages/register/register.uvue:272")
                            ...
                            console.error("Unexpected error during registration attempt:", error, " at pages/register/register.uvue:288")
