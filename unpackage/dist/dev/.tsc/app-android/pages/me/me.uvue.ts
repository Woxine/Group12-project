...
type Order = { __$originalPosition?: UTSSourceMapPosition<"Order", "pages/me/me.uvue", 79, 6>;
	id: string
	bikeId: string
	duration: string
	price: number
...
type UserInfo = { __$originalPosition?: UTSSourceMapPosition<"UserInfo", "pages/me/me.uvue", 88, 6>;
	name: string
	email: string
}
...
const getUserInfo = () => {
	...
	if (userInfoStr != null && userInfoStr != '' && typeof userInfoStr === 'string') {
		try {
			const userInfo = UTSAndroid.consoleDebugError(JSON.parse<UserInfo>(userInfoStr), " at pages/me/me.uvue:106")!!
	...
	uni.request({
		...
		success: (res) => {
			if (res.statusCode >= 200 && res.statusCode < 300) {
				...
				console.error('Failed to fetch orders:', res.statusCode, " at pages/me/me.uvue:165")
				uni.showToast({
		...
		fail: (error) => {
			console.error('Network error:', error, " at pages/me/me.uvue:173")
