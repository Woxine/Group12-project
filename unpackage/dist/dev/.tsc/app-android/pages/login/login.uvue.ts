...
const onSubmit = () => {
	...
	uni.request({
		...
		success: (res) => {
			console.log('Login API Response:', res.data, " at pages/login/login.uvue:102");
			...
			if (res.statusCode === 200) {
				...
				console.log('Login Data:', JSON.stringify(loginData), " at pages/login/login.uvue:120");
				
				// 打印所有可能的字段
				if (loginData != null) {
					console.log('id field:', loginData['id'], " at pages/login/login.uvue:124");
					console.log('userId field:', loginData['userId'], " at pages/login/login.uvue:125");
					console.log('name field:', loginData['name'], " at pages/login/login.uvue:126");
					console.log('email field:', loginData['email'], " at pages/login/login.uvue:127");
					console.log('token field:', loginData['token'], " at pages/login/login.uvue:128");
				}
				...
				if (loginData != null) {
					...
					if (token != null) {
						...
						try {
							...
							console.log('Token saved successfully', " at pages/login/login.uvue:137")
						} catch (e) {
							console.error('Failed to save token:', e, " at pages/login/login.uvue:139")
						}
						...
						try {
							...
							console.log('User ID saved:', userIdToSave, " at pages/login/login.uvue:156")
						} catch (e) {
							console.error('Failed to save user ID:', e, " at pages/login/login.uvue:158")
						}
						...
						const userInfoData = {__$originalPosition: new UTSSourceMapPosition("userInfoData", "pages/login/login.uvue", 165, 13),
							name: userName,
							email: userEmail
						}
						...
						try {
							...
							console.log('User info saved:', userInfoStr, " at pages/login/login.uvue:173")
						} catch (e) {
							console.error('Failed to save user info:', e, " at pages/login/login.uvue:175")
						}
						...
						try {
							...
							console.error('Failed to save role:', e, " at pages/login/login.uvue:185")
						}
						// 强制按字符串比较（UTS 中 JSON 取出的值可能与字面量 === 不相等，用 indexOf 判定）
						...
						console.log('Login role:', roleForCheck, 'isAdmin:', isAdmin, 'targetUrl:', targetUrl, " at pages/login/login.uvue:191")
						
						uni.showToast({
							title: isAdmin ? 'Admin login' : 'Login successful!',
							icon: 'success'
						});
			...
			console.error('Login failed. Status:', res.statusCode, 'Response:', JSON.stringify(retObj), " at pages/login/login.uvue:222");
		...
		fail: (err) => {
			console.error('Login request failed:', err, " at pages/login/login.uvue:231");
			uni.showToast({
