...
const loadData = () => {
	...
	uni.request({
		url: BASE_URL + '/api/v1/admin/revenue/duration-week',
		method: 'GET',
		header: {
...
return (): any | null => {
  ...
  return _cE("view", _uM({ class: "container" }), [
    ...
    _cE("button", _uM({
      class: _nC(['reload-btn', isLoading.value ? 'reload-btn-disabled' : '']),
      disabled: isLoading.value,
      onClick: loadData
    }), _tD(isLoading.value ? 'Loading...' : 'Reload'), 11 /* TEXT, CLASS, PROPS */, ["disabled"])
  ])
}
}
