export function buildBookingApi(http) {
  return {
    ...
    async createGuest({ token, body }) {
      const { res, body: resp } = await http.post('/api/v1/bookings/guest', { token, json: body });
      return { status: res.status, data: resp?.data ?? resp, body: resp };
    },
    async pay({ token, bookingId, body }) {
      const { res, body: resp } = await http.post(`/api/v1/bookings/${encodeURIComponent(bookingId)}/pay`, {
        token,
        json: body
      });
      return { status: res.status, data: resp?.data ?? resp, body: resp };
